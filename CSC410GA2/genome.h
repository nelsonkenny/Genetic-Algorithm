...
//output the genes in a readable way to the console. 
  ...
  double calculate_overall_fitness(Pixel*target, int nPixels); // This function takes an array of Pixels and the size of the image(nPixels) and compares    // the target with its own genes. It should check to make sure that the input size nPixels matches nGenes. It returns -1 if failur. Otherwise,it
  //calculates the average difference between the two and returns that value. 
  void set_pixel(int index, Pixel newPixel);//This function takes the newPixel and set the Pixel at position index to be equal to it. The function should
  //check to make sure that index is valid (between 0 and nGenes) and that the rgb values in newPixel (between 0 and 255 inclusive).    
};
#endif
